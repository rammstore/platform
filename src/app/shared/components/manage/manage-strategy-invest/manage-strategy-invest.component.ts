...
export class ManageStrategyInvestComponent implements OnInit, OnDestroy {
  ...
  buildForm(): void {
    this.form = this.fb.group({
      amount: [(this.authData.getWallets()[0].getEquity() / 10).toFixed(2), [Validators.required, Validators.min(0), Validators.max(this.authData.getWallets()[0].getEquity()), Validators.pattern('^\d*(\.\d{0,2})?$')]],
