...
export class ManageAccountFundComponent implements OnInit, OnDestroy {
  ...
  buildForm(): void {
    this.form = this.fb.group({
      amount: [
        ...
        [Validators.required, Validators.min(0), Validators.max(this.wallet.getAvailableMoney()), Validators.pattern('^[0-9]+([\\,\\.][0-9]{1,2})?$')]
      ]
    });
  }
  ...
  submit(): void {
    ...
    this.dataService.fundAccount(this.account.id, this.form.get('amount').value, this.methodName, this.methodArgs, this.updateStatus)
