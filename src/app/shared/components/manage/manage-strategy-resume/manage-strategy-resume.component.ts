...
export class ManageStrategyResumeComponent implements OnInit, OnDestroy {
  ...
  resume(): void {
    ...
    const queries: any[] = [this.dataService.resumeStrategy(this.strategy.id, this.methodName, this.methodArgs, this.updateStatus)];
    ...
    if (values.amount) {
      debugger
      queries.push(this.dataService.fundAccount(this.strategy.account.id, values.amount, this.methodName, this.methodArgs, this.updateStatus));
    }
    ...
    if (newObj.protection || newObj.target || newObj.factor) {
      debugger
      queries.push(this.dataService.changeAccountProfile(this.strategy.account.id, newObj, this.methodName, this.methodArgs, this.updateStatus));
    }
